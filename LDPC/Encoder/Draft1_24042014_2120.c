...
/**************Procedure Declarations*************/
 ...
 void AddNoise(char noised1, char noised2);
 /***********************************************/
...
unsigned short j, k;
 ...
 /* if (AWGN_flag && !one_run)// && (HighNibble>0) && (LowNibble>0)) {
     {
     j = 0;
     UART1_Write_Text("Adding Noise:\r\n");
     while(j < (i-1)) {
     do randomNumber = rand();
     while (randomNumber >126);
     rxarray[16+j] = rxarray[j] + randomNumber;
     UART1_Write(rxarray[16+j]);
     j++;
     } // end while(rxarray[i])UART1_Write_Text("... sent.\r\n> ");       //send encoded message to Rx
     one_run = 1;
  } */
...
void AddNoise(char noised1, char noised2) {
     ...
     noised1= noised1 + randomNumber;
     noised2 = noised2 + 85;         //constant random number, hopefully constant error

UART1_Write(HighNibble);        //end of text
UART1_Write_Text(" and ");
UART1_Write(LowNibble);

UART1_Write_Text(" With Noise: (");
UART1_Write(2);          //start of text
UART1_Write(noised1);
UART1_Write(noised2);
UART1_Write(3);

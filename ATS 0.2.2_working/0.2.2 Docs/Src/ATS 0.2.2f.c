...
0.2.2    : ATS Software Cleaned up. for Documentation Purposes.
...
// ALL Important Functions defined and/or declared in "Initializers.c"
...
/*Input-Output Table:
     ...
     *  RA3  |Output |  N/A                      |
     *  RA4  |Input  |  Gen Feedback             |Sense that Gen is ON
...
while(1) {                           //Main Endless Loop
     ...
     /* NOTE: When the Gen Cools down, you have to Start
...
void crank() { //Crank till there's feedback. 5X2 seconds before failing.
          ...
          delay_ms(1000);    //Just wait...
           if (!Run_flag) {  //if the Gen is running, leave and Poll
              if (!CoolDown_Flag) {
                GenStop = 1;
                GenOn = 1;
                crank();   //...and run the Generator Start Routine.
